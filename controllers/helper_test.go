...
func Test_generateManifestWorkName(t *testing.T) {
	type args struct {
		mbac mrbacv1alpha1.ManagedRBAC
	}
	tests := []struct {
		name string
		args args
...
func Test_buildManifestWork(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := buildManifestWork(tt.args.managedRBAC, tt.args.manifestWorkName, tt.args.clusterRole, tt.args.clusterRoleBinding,
			...
			// check work owner
			if got.OwnerReferences[0].Kind != "ManagedRBAC" {
				t.Errorf("buildManifestWork() owner = %v, want %v", got.OwnerReferences[0].Kind, "ManagedRBAC")
			}
